...
entry_group_callback(AvahiEntryGroup *g, AvahiEntryGroupState state,
    ...
    switch(state) {
        ...
        case AVAHI_ENTRY_GROUP_FAILURE:
            ...
            avahi_simple_poll_quit(dm->simple_poll);
            break;
...
addMdnsRecordForNetworkLayer(UA_DiscoveryManager *dm, const UA_String serverName,
    ...
    if(hostname.length == 0) {
        ...
        const char *hoststr = avahi_client_get_host_name(dm->client);
        if(!hoststr) {
            UA_LOG_WARNING(dm->sc.server->config.logging, UA_LOGCATEGORY_DISCOVERY,
...
resolve_callback(AvahiServiceResolver *r, AVAHI_GCC_UNUSED AvahiIfIndex interface,
    ...
    switch(event) {
        ...
        case AVAHI_RESOLVER_FOUND: {
            ...
            /* Ignore own name */
            if(UA_String_equal(&dm->selfMdnsRecord, &serverName)) {
    ...
    switch(event) {
        ...
        case AVAHI_BROWSER_NEW:
            ...
            if(!(avahi_service_resolver_new(
                   dm->client, interface, protocol, name, type, domain, AVAHI_PROTO_INET,
                   AVAHI_LOOKUP_USE_MULTICAST, resolve_callback, dm))) {
                UA_LOG_INFO(
                    dm->sc.server->config.logging, UA_LOGCATEGORY_DISCOVERY,
                    "Failed to resolve service '%s' of type '%s' in domain '%s': %s",
                    name, type, domain, avahi_strerror(avahi_client_errno(dm->client)));
            }
            break;
...
UA_DiscoveryManager_startMulticast(UA_DiscoveryManager *dm) {
    int error;
    dm->simple_poll = avahi_simple_poll_new();
    if(!dm->simple_poll) {
        UA_LOG_ERROR(dm->sc.server->config.logging, UA_LOGCATEGORY_DISCOVERY,
    ...
    dm->client = avahi_client_new(avahi_simple_poll_get(dm->simple_poll),
    ...
    dm->browser = avahi_service_browser_new(dm->client, AVAHI_IF_UNSPEC, AVAHI_PROTO_INET,
...
UA_DiscoveryManager_stopMulticast(UA_DiscoveryManager *dm) {
    ...
    if(dm->browser)
        avahi_service_browser_free(dm->browser);
    if(dm->client)
        avahi_client_free(dm->client);
    if(dm->simple_poll)
        avahi_simple_poll_free(dm->simple_poll);
...
void UA_DiscoveryManager_mdnsCyclicTimer(UA_Server *server, void *data) {
    int ret = avahi_simple_poll_iterate(((UA_DiscoveryManager*)data)->simple_poll, 0);
    if(ret < 0) {
...
UA_Discovery_addRecord(UA_DiscoveryManager *dm, const UA_String servername,
    ...
    if(isSelf && dm->selfMdnsRecord.length == 0) {
        dm->selfMdnsRecord = UA_STRING_ALLOC(serviceDomain);
        if(!dm->selfMdnsRecord.data)
            return UA_STATUSCODE_BADOUTOFMEMORY;
    }
    ...
    if(retval != UA_STATUSCODE_BADALREADYEXISTS) {
        listEntry->group = avahi_entry_group_new(dm->client, entry_group_callback, dm);
        if(!listEntry->group) {
            UA_free(listEntry);
